...
@section('extra-style')
    <link rel="stylesheet" href="{{ asset('assets/styles/auth/banner.css') }}">
@endsection

@section('content')
    <div class="page__header__container">
        <h1 class="page__header__title">Gestion des bannières</h1>
    </div>
    <div class="page__content__container">

        <a href="#" class="primary__link__btn" onclick="modalOpener(this)" data-target="#addBanner"><i class="fa fa-plus"></i> Ajouter une bannière</a>

        @include('includes.auth.flash')

        <div class="banner__card__container">
            @foreach($banners as $banner)
                <div class="banner__card">
                    <figure class="banner__image">
                        <img src="{{ asset('storage/' . $banner->image) }}" alt="Image de la banniere">
                        <div class="banner__description">{{ $banner->title }}</div>
                    </figure>
                    <div class="banner__content">
                        <a href="#!" class="edit__button" onclick="modalOpener(this)" data-target="#addBanner1">Modifier</a>
                        <a href="#!" onclick="event.preventDefault(); confirm('Etes-vous sûr de vouloir supprimer cette bannière ?') ? document.getElementById('deleteBanner{{ $banner->id }}').submit() : ''" class="delete__button">Supprimer</a>
                        <form action="{{ route('auth.banners.destroy', $banner) }}" method="POST" id="deleteBanner{{ $banner->id }}">
                            @csrf
                            @method("DELETE")
                        </form>
                    </div>

                    <div class="modal__container" id="addBanner1">
                        <div class="modal">
                            <div class="modal__body">
                                <form action="{{ route('auth.banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PATCH')
        ...
        <div class="modal__container" id="addBanner">
            <div class="modal">
                <div class="modal__body">
                    <form action="{{ route('auth.banners.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form__group">
                            <label for="title" class="form__label">Titre</label>
